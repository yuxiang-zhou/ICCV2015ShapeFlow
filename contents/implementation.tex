...
Using the SVS functions built from training data, the next step of our pipeline is to establish dense correspondences between all these functions. We propose to do this procedure jointly, by using the correspondence basis. This procedure shares similarities with multiframe optical flow estimation \cite{Garg:2013hu} and we call it Shape flow. We build upon robust methods for  optical flow estimation. Optical flow estimation typically works based on the assumptions of brightness or colour constancy and motion smoothness. However, in terms of shapes, neither constant illumination nor continuous motion exists in sequence of shapes as all training data are independent.
...
Instead of the \emph{motion} basis used in multiframe optical flow formulation of \cite{Garg:2013hu}, we build a \emph{correspondence} basis that introduces constrains in how points of different shapes are matched to each other. By considering a reference shape, every pixel of the image that represents this shape is matched with its corresponding position and in this way defines a \emph{correspondence vector}. This vector consists of the 2D locations of the specific point in all shape images. To form this vector, the training shapes are arranged in an arbitrary order. Similarly to the order of training samples when Principal Component Analysis (PCA) is applied, this order does not affect the result of our method and any re-ordering would produce exactly the same results.


The correspondence basis that we build constrains these correspondence vectors to lie on a subspace and in this way acts as a low-rank prior that enforces coherency of the shape registration result over the whole training dataset of shapes. To effectively build this basis, we first apply Non-rigid Iterative Closest Point (NICP)\cite{Amber2007} between the cloud of sparse annotated landmarks in the reference shape and the one of every shape of the training set. NICP deforms the cloud of points of every shape to match the points of the reference shape. This yields an initial estimation of the correspondence vectors on the sparse locations of annotated landmarks on the reference shape. To make this correspondence vectors estimation dense (i.e.~for every pixel of the reference shape), we apply Thin-Plate Splines (TPS)\cite{Bookstein1989} interpolation. This initial estimation of dense correspondence vectors define for every training shape a warping function that registers it with the reference shape and equivalently an initial ``shape flow'', i.e.~ an ``optical flow'' between the reference shape and every training shape.

The correspondence basis is found by applying PCA on the result of TPS interpolation and keeping only the first $R$ principal components. Formally, let $F$ be the number of training shapes and $n=1,\ldots,F$ be the training shape index. Also, let $\boldsymbol{q}_1(n),\ldots,\boldsymbol{q}_R(n):
\{1,\ldots,F\} \rightarrow \R^2$ be the $R$ orthonormal elements of the correspondence basis, where $\boldsymbol{q}_i(n)$ is the displacement vector that matches the points of the reference shape with the points of the $n$-th training shape shape, according to the variation described from the $i$-th correspondence basis element. Note that the basis elements $\boldsymbol{q}_i(n)$ are independent from the point location.

In addition, let .. be the image domain where the reference and training shapes are defined and ... denote the point location. We denote the shape flow result as ...,  where $u_n(x)$ is the displacement vector that matches the point ... of the image of the reference shape with its corresponding location at the image of the $n$-th training shape.
...
where $\bm{v_i}(\bm{x})$ is the weight that needs to be applied on the $i$-th correspondence basis element, in order to get the correspondence vector for the point location $\bm{x}$. In other words, the shape flow for every point $\bm{x}$ is described as linear combination of basis elements that is controlled by the coefficients $\bm{v_i}(\bm{x})$ that depend on $\bm{x}$.
The values of the $i$-th coefficient for all the points $\bm{v_i}(\bm{x})$ can be interpreted as an image defined on $\Omega$. Using the correspondence basis, the determination of the shape flow boils down to the determination of the set of coefficients $\bm{v_i}(\bm{x})$
...
a set of warping functions that give an initial registration of every training shape with the reference shape.


Performs the algorithm for all training data will get an implicit point correlation between sparse shapes.
...
where $\bm{q_i}(n)$ are low rank components with $R \ll 2F$ and $\bm{v_i}(x)$ weighted each component with dependencies on $x$. Simpler expression shown below:
\begin{equation}
    \bm{u_n}(\bm{x})=\sum_{i=1}^R\bm{q_i}(n)\bm{v_i}(\bm{x})+\bm{\varepsilon_n}(\bm{x})
\end{equation}
