\begin{figure*}[t!]
    ...
    \caption{\textcolor{red}{Schematic description of the proposed pipeline. Figure best viewed by zooming in.}}
    \label{fig:archi}
\end{figure*}
...
\textcolor{red}{The proposed pipeline is depicted in Figure~\ref{fig:archi}. It takes as input a set of training images of a particular object class, along with inconsistent point and/or curve line shape annotations.
...
\begin{figure}[b!]
    ...
    \caption{\textcolor{red}{Exemplar SVS shape representation. A similar SVS image is obtained from different annotations of the same shape.}}
    \label{fig:build_svs}
\end{figure}
...
\textcolor{red}{In addition, let $\Omega \subset \R^2$ be the image domain of the SVS images and $\bx$ denote the point location. We denote the shape flow result as $\bm{u}_n(\bx):\Omega\times \{1,\ldots,F\}
...
\textcolor{red}{Using the constructed correspondence basis, the shape flow can be approximated as:
...
\begin{figure}[t!]
    ...
    \end{subfigure}
    \caption{\textcolor{red}{Exemplar deformation fields obtained using the proposed pipeline. Figure best viewed by zooming in.}}
...
\textcolor{red}{Following \cite{Garg:2013hu}, we propose to estimate the shape flow over all training SVS images $\bm{d}_n(\bx)$ by minimizing the following energy:
\begin{align}
E_{sf} & =\alpha
...
\textcolor{red}{The \textbf{first term} of the above energy \eqref{eq:costfunc} is a data attachment term
...
\textcolor{red}{Figure~\ref{fig:deformationfield} shows some examples of deformation fields derived from the estimated shape flow computed by the aforementioned method. These results correspond to exemplar training shapes in the case of a face dataset.}
